...
The goal of moving multi-epoch to the same frame could be accomplished by building transformation models from one epoch to another. Generally, every inter-epoch image pair would be matched, followed by a RANSAC routine to recover the best transformation model between each image pair.
...
\label{Datasets}
%Frejus, Pezenas, Kobe\\
We tested our rough co-registration methods on the 4 sets of datasets which are elaborated in Chapter~\ref{chap:ApplicationsAndDatasets}: Fr{\'e}jus, Pezenas, Kobe and Alberona.
...
%\end{enumerate}
Experiments comparing $SIFT_{Default}$ and $SIFT_{Adapted}$ on methods ${ImgPairs}$, ${Ortho}$ and ${DSM}$ are displayed in section ~\ref{Compare2SIFTs}, %Appendix ~\ref{chap:appendix1}, 
...
As the results showed similar pattern, for the sake of simplicity, we only show the results of 2 sets of datasets (i.e. Fr{\'e}jus and Alberona), as the former represents challenging case with drastic scene changes and the latter stands for general case, and we move the results of Kobe as well as Pezenas to Chapter~\ref{chap:appendixA} to keep the current section tidy.\\
...
According to the absolute average value $|\mu|$ displayed in Table~\ref{RoughDoDStatistic}, method $SuperGlue_{DSM}$ performed the best for matching Fr{\'e}jus 1954 and 1966 to 2014, where drastic scene changes present. %For the case of matching Fr{\'e}jus 1970 to 2014, as the 2 epochs are with shorter time gap,\\
Similar pattern is present in other datasets (Fr{\'e}jus, Pezenas and Kobe), for more details please refer to section~\ref{sec:RoughDoD}.\\

It is worth noting that the \ac{DoD}s between Fr{\'e}jus 1954 and 2014 based on method $SIFT_{DSM}$ (i.e. \ref{DoDFrejus} (f)) is accompanied with specifically obvious dome effect, with its absolute average value $|\mu|$ reached 8.04m as shown in Table \ref{RoughDoDStatistic}. This is consistent with the results shown in Section \ref{sec:MatchVizMainBody}, as its inlier ratio of matches in the RANSAC procedure is too low to guarantee reliable rough co-registration.
...
We applied all the 6 methods on 3 datasets (Fr{\'e}jus, Pezenas, Kobe and Alberona), including the cases of (1) matching aerial epochs only and (2) matching aerial and satellite epochs mixed.

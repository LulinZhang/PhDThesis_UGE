...
To improve efficiency, all input images are downsampled by a factor of 3 beforehand, except for dataset Alberona as it consists of few images. To calculate the DSMs and orthophotos, we further downsample the images by a factor of 8, which amounts to a total downsampling factor of 24 with respect to the input images. For example, the images in Fr{\'e}jus 1970 are downsampled from [8766, 8763] to [365, 365] for calculating DSMs and orthophotos. As the goal of rough co-registration is to get robust rather than precise matches, a low resolution DSM/orthophoto is good enough and keeps the computational cost low. \\
...
For the sake of simplicity, the matches visualization and \ac{DoD}s of only Alberona are displayed in this chapter, the results of Fr{\'e}jus, Pezenas and Kobe are demonstrated in Section~\ref{chap:appendixA}.
...
The visualization of \ac{DoD}s for dataset Alberona is demonstrated in~\ref{DoDAlberona}. The corresponding statistical information is displayed in ~\ref{AlberonaDoDStatistic}.\\
...
Similar pattern is present in other datasets (Fr{\'e}jus, Pezenas and Kobe), for more details please refer to section~\ref{sec:matchViz}.
...
\begin{table}%[H]
	...
	\end{tabular}
	\caption{Average value $\mu$, standard deviation $\sigma$, and absolute average value $|\mu|$ of all the \ac{DoD}s in Figure~\ref{DoDAlberona}.}
	\label{AlberonaDoDStatistic}
\end{table}

...
\begin{enumerate}
	...
	\item Remove candidate matches whose scales and rotations computed by SIFT are out of range [(1-$Th_{scl}$)$\times$$R_{scl}$, (1+$Th_{scl}$)$\times$$R_{scl}$] and [$D_{rot}$-$Th_{rot}$, $D_{rot}$+$Th_{rot}$] ;% computed from image orientations and the co-registered \ac{DSM} (i.e., step 1);
...
For matches visualization and \ac{DoD}, as the results show similar pattern on different datasets, we only display the results of Fr{\'e}jus and Alberona in the current section for the sake of simplicity, and move the results of Kobe as well as Pezenas to Appendix~\ref{chap:appendixB}.\\

%\subsection{Comparison}
%\subsubsection{Matches visualization}
\paragraph{Matches visualization.}
\label{matchVizMainBody}
For each dataset, we match every possible combination of 2 epochs with 4 variants (i.e., \ding{172} $Patch_{SpGDSM}$, \ding{173} $Guided_{SpGDSM}$, \ding{174} $Patch_{SIFTDSM}$ and \ding{175} $Guided_{SIFTDSM}$). 
...
maps of Kobe dataset as well as the ground truth Gd provided by the Geospatial Information Authority of Japan %~\cite{ian1996morphological1}
...
%			\begin{minipage}[t]{1\linewidth}
%				\centering
%				\includegraphics[width=9cm,angle=2]{images/Chapitre4/Kobe-faultmap.png}
%			\end{minipage}%
%		}
...
%			\begin{minipage}[t]{1\linewidth}
%				\centering
%				\includegraphics[width=9cm,trim=1250 400 100 570,clip]{images/Chapitre4/Gd1991_Patch_SpGDSM.png}
%			\end{minipage}%
%		}
%		\subfigure[Gd$_{Kobe}^{Guided_{SpGDSM}}$]{
%			\begin{minipage}[t]{1\linewidth}
%				\centering
%				\includegraphics[width=9cm,trim=1250 360 100 570,clip]{images/Chapitre4/Gd1991_Guided_SpGDSM.png}
%			\end{minipage}%
%		}\\
...
\begin{figure*}[htbp]
	\begin{center}
		\subfigure[Gd$_{Kobe}^{{GT}}$]{
			\begin{minipage}[t]{1\linewidth}
				\centering
				\includegraphics[width=7.8cm,angle=0]{images/Chapitre4/GT.png}
			\end{minipage}%
		}
		\subfigure[Gd$_{Kobe}^{{SpGDSM}}$]{
			\begin{minipage}[t]{1\linewidth}
				\centering
				\includegraphics[width=7.8cm]{images/Chapitre4/SpGDSM.png}
			\end{minipage}%
		}
		\subfigure[Gd$_{Kobe}^{Patch_{SpGDSM}}$]{
			\begin{minipage}[t]{1\linewidth}
				\centering
				\includegraphics[width=7.8cm,trim=0 0 0 20,clip]{images/Chapitre4/patch-spgdsm.png}
			\end{minipage}%
		}
		\subfigure[Gd$_{Kobe}^{Guided_{SpGDSM}}$]{
			\begin{minipage}[t]{1\linewidth}
				\centering
				\includegraphics[width=7.8cm,trim=0 0 0 20,clip]{images/Chapitre4/guided-spgdsm.png}
			\end{minipage}%
		}\\
		\subfigure[Gd$_{Kobe}^{{SIFTDSM}}$]{
			\begin{minipage}[t]{1\linewidth}
				\centering
				\includegraphics[width=7.8cm]{images/Chapitre4/SIFTDSM.png}
			\end{minipage}%
		}
		\subfigure[Gd$_{Kobe}^{Patch_{SIFTDSM}}$]{
			\begin{minipage}[t]{1\linewidth}
				\centering
				\includegraphics[width=7.8cm,trim=0 0 0 20,clip]{images/Chapitre4/patch-siftdsm.png}
			\end{minipage}%
		}
		\subfigure[Gd$_{Kobe}^{Guided_{SIFTDSM}}$]{
			\begin{minipage}[t]{1\linewidth}
				\centering
				\includegraphics[width=7.8cm,trim=0 0 0 20,clip]{images/Chapitre4/guided-siftdsm.png}
			\end{minipage}%
		}
		...
		\subfigure[Gd legend]{
			\begin{minipage}[t]{1\linewidth}
				\centering
				\includegraphics[width=7.8cm]{images/Chapitre4/legend-Gd.png}
			\end{minipage}%
		}
...
%For the Gds, according to the \ac{GT} (i.e., Figure~\ref{GdKobe} (a)), a striking north $30^{\circ}$-$60^{\circ}$ east rupture occurred along the northeast-southwest coastline across $ \sim $18 km. The \ac{GT} is produced with a lot of manual work, which is very laborious. but ours is completely automatic. According to the ground truth, there is a fault here caused by the earthquake, and the displacement is indicated as small arrows. on the left side of the fault, the arrows are upward, and on the right side they are downward. In ours, the fault is indicated as dashed line here. And the displacement is indicated as colors, blue represents upward and red represents downward. As can be seen, our method recovered the same signal as ground truth, which is upward on the left side and downward on the right.

...
As can be seen, for Pezenas free epochs $E_f$, the most weak varaints $SIFT_{ImgPairs}$ and $SIFT_{Ortho}$ succeeded on matching them to the aerial reference epoch $E_r$, since Pezenas showed neither drastic scene changes nor different image tones, which lowered the matching difficulty. However, $SIFT_{Ortho}$ failed on matching Pezenas $E_f$ to the satellite reference epoch $E_r$, since satellite epoch not only has more limited zone overlapped with the free epochs, especially for epoch 1971, but also is covered with clouds. For the rest 4 variants, we can see the same pattern as the results of Fr{\'e}jus and Alberona (c.f. Section \ref{para:Discussion}).
%	\begin{itemize}
...
\begin{figure*}[htbp]
	\begin{center}
		...
		\caption{{\scriptsize \ac{DoD}s between free epoch \textbf{Pezenas 1971, 1981} and reference aerial epoch \textbf{2015} with variants $SuperGlue_{ImgPairs}$ (a, g), $SuperGlue_{Ortho}$ (b, h), $SuperGlue_{DSM}$ (c, i), $SIFT_{ImgPairs}$ (d, j), $SIFT_{Ortho}$ (e, k) and $SIFT_{DSM}$ (f, l). The prohibition sign means the corresponding variant failed.}}
		\label{DoDPezenas}
	\end{center}
\end{figure*} 
...
\begin{figure*}[htbp]
	\begin{center}
		...
		\caption{{\scriptsize \ac{DoD}s between free epoch \textbf{Pezenas 1971, 1981} and reference satellite epoch \textbf{2014} with variants $SuperGlue_{Ortho}$ (a, e), $SuperGlue_{DSM}$ (b, f), $SIFT_{Ortho}$ (c, g) and $SIFT_{DSM}$ (d, h). The holes among them are areas covered with clouds which are masked out. The prohibition sign means the corresponding variant failed.}}
...
\begin{figure*}[htbp]
	\begin{center}
		...
		\caption{{\scriptsize \ac{DoD}s between free epoch \textbf{Kobe 1991} and reference epoch \textbf{1995} with variants $SuperGlue_{ImgPairs}$ (a), $SuperGlue_{Ortho}$ (b), $SuperGlue_{DSM}$ (c), $SIFT_{ImgPairs}$ (d), $SIFT_{Ortho}$ (e) and $SIFT_{DSM}$ (f). The prohibition sign means the corresponding variant failed.}}
		\label{DoDKobe}
	\end{center}
\end{figure*} 
